...
in all database crypto lists (accounts, channels, groups) and alarms.
"""
...
def main():
    ...
    rows = db.execute(
        True,
        f"SELECT {db.ACCOUNT_ID}, {db.ACCOUNT_CRYPTOS}, {db.ACCOUNT_CALC_CRYPTOS} FROM {db.TABLE_ACCOUNTS}",
    )
    if rows:
        for row in rows:
            stats["accounts_checked"] += 1
            chat_id, crypto_raw, calc_crypto_raw = row

            new_crypto, crypto_rep, crypto_rem = migrate_token_list(crypto_raw)
            new_calc, calc_rep, calc_rem = migrate_token_list(calc_crypto_raw)

            total_replaced = crypto_rep + calc_rep
            total_removed = crypto_rem + calc_rem

            if total_replaced > 0 or total_removed > 0:
                db.execute(
                    False,
                    f"UPDATE {db.TABLE_ACCOUNTS} SET {db.ACCOUNT_CRYPTOS}=%s, {db.ACCOUNT_CALC_CRYPTOS}=%s WHERE {db.ACCOUNT_ID}=%s",
                    new_crypto,
                    new_calc,
                    chat_id,
                )
    ...
    rows = db.execute(
        True,
        f"SELECT {db.CHANNEL_ID}, {db.CHANNEL_COINS} FROM {db.TABLE_CHANNELS}",
    )
    if rows:
        for row in rows:
            stats["channels_checked"] += 1
            channel_id, cryptos_raw = row

            new_cryptos, replaced, removed = migrate_token_list(cryptos_raw)

            if replaced > 0 or removed > 0:
                db.execute(
                    False,
                    f"UPDATE {db.TABLE_CHANNELS} SET {db.CHANNEL_COINS}=%s WHERE {db.CHANNEL_ID}=%s",
                    new_cryptos,
                    channel_id,
                )
    ...
    rows = db.execute(
        True,
        f"SELECT {db.GROUP_ID}, {db.GROUP_COINS} FROM {db.TABLE_GROUPS}",
    )
    if rows:
        for row in rows:
            stats["groups_checked"] += 1
            group_id, cryptos_raw = row
            ...
            if replaced > 0 or removed > 0:
                db.execute(
                    False,
                    f"UPDATE {db.TABLE_GROUPS} SET {db.GROUP_COINS}=%s WHERE {db.GROUP_ID}=%s",
                    new_cryptos,
                    group_id,
                )
