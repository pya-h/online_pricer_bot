...
class DatabaseInterface:
    ...
    DATE_FORMAT = '%Y-%m-%d'
    ACCOUNT_COLUMNS = (ACCOUNT_ID, ACCOUNT_CURRENCIES, ACCOUNT_CRYPTOS, ACCOUNT_LAST_INTERACTION, ACCOUNT_PLUS_END_DATE, ACCOUNT_PLUS_PLAN_ID, ACCOUNT_STATE, ACCOUNT_CACHE, ACCOUNT_IS_ADMIN, ACCOUNT_LANGUAGE) =\
        ('id', 'currencies', 'cryptos', 'last_interaction', 'plus_end_date', 'plus_plan_id', 'state', 'cache', 'admin', 'language')
        ...
        try:
            ...
            if not cursor.execute(f"SELECT name from sqlite_master WHERE name='{self.TABLE_ACCOUNTS}'").fetchone():
                query = f"CREATE TABLE {self.TABLE_ACCOUNTS} ({self.ACCOUNT_ID} INTEGER PRIMARY KEY," +\
                    f"{self.ACCOUNT_CURRENCIES} TEXT, {self.ACCOUNT_CRYPTOS} TEXT, {self.ACCOUNT_LAST_INTERACTION} DATE, " +\
                ...
                # create table account
                cursor.execute(query)
                log(f"PLUS Database {self.TABLE_ACCOUNTS} table created successfuly.", category_name='plus_info')
            else: # TEMP-*****
                self.sync()
    ...
    def add(self, account):
        ...
        try:
            ...
            self.execute(False, query, account.chat_id, account.str_desired_currencies(), account.str_desired_coins(), account.last_interaction.strftime(self.DATE_FORMAT), \
                                   account.plus_end_date, account.plus_plan_id, account.state.value, account.cache, account.is_admin, account.language)
    ...
    def update(self, account):
        ...
        if cursor.fetchone(): # if account with his chat id has been saved before in the database
            ...
            cursor.execute(f'UPDATE {self.TABLE_ACCOUNTS} SET {columns_to_set} WHERE {self.ACCOUNT_ID}=?', \
                (account.str_desired_currencies(), account.str_desired_coins(), account.last_interaction.strftime(self.DATE_FORMAT), \
                 account.plus_end_date.strftime(self.DATE_FORMAT) if account.plus_end_date else None, account.plus_plan_id,
            ...
            cursor.execute(f"INSERT INTO {self.TABLE_ACCOUNTS} ({columns}) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", \
                (account.chat_id, account.str_desired_currencies(), account.str_desired_coins(), account.last_interaction.strftime(self.DATE_FORMAT), \
                 account.plus_end_date.strftime(self.DATE_FORMAT) if account.plus_end_date else None, account.plus_plan_id, 
    ...
    def upgrade_account(self, account, plus_plan):  # use plus mode
        ...
        self.execute(False, f'UPDATE {self.TABLE_ACCOUNTS} SET {self.ACCOUNT_PLUS_END_DATE}=?, {self.ACCOUNT_PLUS_PLAN_ID}=? WHERE {self.ACCOUNT_ID}=?', \
    ...
    def define_plus_plan(self, title: str, titile_en: str, duration_in_months: int, price: float, price_currency: str = "USDT", plus_level: int = 1):
        fields = ', '.join(self.PLUS_PLANS_COLUMNS[1:-2])  # in creation mode admin just defines persian title and description
        # if he wants to add english texts, he should go to edit menu
        self.execute(f"INSERT INTO {self.TABLE_PLUS_PLANS} ({fields}) VALUES (?, ?, ?, ?, ?, ?)", \
            price, price_currency, duration_in_months, plus_level, title, titile_en)

    def get_plus_plan(self, plus_plan_id: int):
        vplans = self.execute(True, f"SELECT * FROM {self.TABLE_PLUS_PLANS} WHERE {self.PLUS_PLAN_ID}=? LIMIT 1", plus_plan_id)
    ...
    def update_payment(self, payment):
        connection = sqlite3.connect(self._name)
        cursor = connection.cursor()
        cursor.execute(f"SELECT * FROM {self.TABLE_PAYMENTS} WHERE {self.PAYMENT_ORDER_ID}=? LIMIT 1", (payment.order_id, ))
    ...
    def get_payment(self, order_id):
        columns = ', '.join(self.PAYMENT_COLUMNS)
        return self.execute(True, f"SELECT {columns} from {self.TABLE_PAYMENTS} WHERE {self.PAYMENT_ORDER_ID}=? ", order_id)
