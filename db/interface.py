...
class DatabaseInterface:
    ...
    DATE_FORMAT = '%Y-%m-%d'
    ACCOUNT_COLUMNS = (ACCOUNT_ID, ACCOUNT_CURRENCIES, ACCOUNT_CRYPTOS, ACCOUNT_LAST_INTERACTION, ACCOUNT_PLUS_END_DATE,
                       ACCOUNT_PLUS_PLAN_ID, ACCOUNT_STATE, ACCOUNT_CACHE, ACCOUNT_IS_ADMIN, ACCOUNT_LANGUAGE) = \
        ('id', 'currencies', 'cryptos', 'last_interaction', 'plus_end_date', 'plus_plan_id', 'state', 'cache', 'admin',
         'language')
    ...
    def add(self, account):
        ...
        try:
            ...
            self.execute(False, query, account.chat_id, account.str_desired_currencies(), account.str_desired_coins(),
                         account.last_interaction.strftime(self.DATE_FORMAT), \
                         account.plus_end_date, account.plus_plan_id, account.state.value, account.cache,
                         account.is_admin, account.language)
