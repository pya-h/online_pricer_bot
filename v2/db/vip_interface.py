...
class VIPDatabaseInterface(DatabaseInterface):
    ...
    ACCOUNT_ALL_FIELDS = f'({DatabaseInterface.ACCOUNT_ID}, {DatabaseInterface.ACCOUNT_CURRENCIES}, {DatabaseInterface.ACCOUNT_CRYPTOS}, {DatabaseInterface.ACCOUNT_LAST_INTERACTION}, {ACCOUNT_VIP_END_DATE}, {DatabaseInterface.ACCOUNT_LANGUAGE})'
        ...
        try:
            ...
            if not cursor.execute(f"SELECT name from sqlite_master WHERE name='{VIPDatabaseInterface.TABLE_ACCOUNTS}'").fetchone():
                query = f"CREATE TABLE {VIPDatabaseInterface.TABLE_ACCOUNTS} ({VIPDatabaseInterface.ACCOUNT_ID} INTEGER PRIMARY KEY," +\
                    f"{VIPDatabaseInterface.ACCOUNT_CURRENCIES} TEXT, {VIPDatabaseInterface.ACCOUNT_CRYPTOS} TEXT, {VIPDatabaseInterface.ACCOUNT_LAST_INTERACTION} DATE, {VIPDatabaseInterface.ACCOUNT_VIP_END_DATE} DATE, {VIPDatabaseInterface.ACCOUNT_LANGUAGE} TEXT)"
